...
### Changed
- **Onboarding menu bar hint arrow** — arrow now points up-and-to-the-left to better gesture toward where the Cloom menu bar icon typically lands (left half of the menu bar, past the macOS app menus). Bounce animation mirrors the new direction.
...
### Added
- **Creator Mode** — Settings toggle that includes Cloom's own floating UI panels (webcam bubble, annotations, teleprompter, keystroke overlay, zoom, recording toolbar) in screen recordings. When enabled, SCStream captures panels directly instead of software compositing. Great for tutorials and walkthroughs.
- Creator Mode entry in the in-app Features directory.

### Changed
- Replaced deprecated `NSWindowSharingType.readWrite` with `.readOnly` across all floating panels.
